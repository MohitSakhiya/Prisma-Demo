...
export const addCategory = async (req: Request, res: Response) => {
    ...
    if (!(category_name)) return res.status(400).json({ message: 'Category Name Required', success: false, data: {} });
    ...
    return res.status(200).json({ message: 'Category Add Successfully', success: true, data: { category } });
}
